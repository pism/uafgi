...
def compute(geometry_file, velocity_file, grid_fivar, output_file, ice_softness=3.1689e-24):
    """
    geometry_file:
    ...
    config.set_number("flow_law.isothermal_Glen.ice_softness", ice_softness)

    # get grid information from the variable "thk" in a file.
    # (Not a full-blown 3D grid, by getting from thk which is a 2D variable)
    # grid = PISM.IceGrid.FromFile(ctx.ctx, geometry_file, ["thk"], PISM.CELL_CENTER)
    grid = PISM.IceGrid.FromFile(ctx.ctx, grid_fivar[0], grid_fivar[1], PISM.CELL_CENTER)
    ...
    # Read number of timesteps from the velocity_file
    f = PISM.File(ctx.com, velocity_file,
        PISM.string_to_backend(ctx.config.get_string("output.format")),
        PISM.PISM_READONLY, ctx.ctx.pio_iosys_id())
    try:
        ntime = f.nrecords()
    finally:
        f.close()

    # Get the time metadata more fully
    ...
    geometry.ice_thickness.regrid(geometry_file)
    geometry.bed_elevation.regrid(geometry_file)
    ...
    try:
        ...
        for timestep in range(0,ntime):
            ...
            ice_velocity.read(velocity_file, timestep)   # 0 ==> first record of that file (if time-dependent)
...
def main():
    ...
    compute_calving(
        geometry_file='bedmachine/BedMachineGreenland-2017-09-20.nc',
        velocity_file=velocity_file,
        grid_fivar=(velocity_file,('vx',)),
        output_file='outputs/calving.nc')
